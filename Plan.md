...
ear:
  ...
  short_blink_max_ms: 250     # Chớp mắt tự nhiên (click trái)
  long_blink_min_ms: 500      # Nhắm lâu (click phải)
  consecutive_frames: 2       # Số frame liên tiếp để xác nhận blink
...
EAR < threshold (0.21)
   ...
   EAR > threshold
        │
        ├── Thời gian < 250ms   → SHORT BLINK → Click trái
        ├── Thời gian 500-2000ms → LONG BLINK  → Click phải
        └── Thời gian > 2000ms  → Bỏ qua (có thể đang đọc)
```
...
// calibration/calibration_data.json
{
  ...
  "samples": [
    {
      "screen_x": 240, "screen_y": 180,
      "gaze_yaw": -28.4, "gaze_pitch": -12.1,
      "n_frames": 20
    }
  ]
}
...
*Tài liệu này dành cho Giải pháp A (L2CS-Net + Calibration Layer). Sau khi hoàn thành, có thể chuyển sang Giải pháp B (Fine-tune) bằng cách dùng data thu được ở Phase 3 làm điểm khởi đầu.*